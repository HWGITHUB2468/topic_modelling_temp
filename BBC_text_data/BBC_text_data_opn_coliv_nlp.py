# -*- coding: utf-8 -*-
"""
Created on Mon Jun 12 08:23:27 2023

@author: brenng


Created on Tue Jun  6 15:05:48 2023

@author: brenng
"""

'''
Purpose: assess the open text question related to the Cost of Living crisis in the OPN survey, 2023

Stages:
    Import packages
    Import data
    Create Relevant Fucntions...pre processing, cleaning, modelling and splitting 
    
Project Details:
    
File pathway:"Z:\Covid\opn_coliv_nlp\
File: OPN2304DK_Main_Income_nlp.csv
Data Column:COL_OwnWords
row count (raw): 2018
...
#from spellchecker import SpellChecker # this old and going to switch to a newer package
...
path_to_file = r'C:\Users\walenh\nlp_temp\topic_modelling_temp\BBC_text_data\bbc_data.csv'
opn_coliv_nlp = pd.read_csv(path_to_file)

opn_coliv_nlp.head(5)
opn_coliv_nlp.shape
opn_coliv_nlp['data'].shape
...
opn_coliv_nlp['data'][14]

# lower casing

opn_coliv_nlp['data'] = opn_coliv_nlp['data'].str.lower()
...
opn_coliv_nlp['data'] = opn_coliv_nlp['data'].apply(remove_punct)

opn_coliv_nlp['data'][14]

opn_coliv_nlp['data'].shape #should be seeing some lines removed here...there are some , and . starts. Might have become blanks...recheck count after blanks removed
...
opn_coliv_nlp = ( 
    opn_coliv_nlp
    .loc[lambda df: ~df['data'].str.startswith('9999')]
    .loc[lambda df: df['data']!='0']
)
...
opn_coliv_nlp['data'] = opn_coliv_nlp['data'].replace(r'^s*$', float('NaN'), regex = True)  # Replace blanks by NaN
opn_coliv_nlp.dropna(subset = ['data'], inplace = True)     # Remove rows with NaN

opn_coliv_nlp['data'].shape # not removing all due to spaces at the start of the blank row...look for a regex that looks for spaces...or look up 'strip' for same purose (don't ID a character and it will remove spaces)
...
nltk.download('punkt')
opn_coliv_nlp['data'][456]
opn_coliv_nlp['data_tokens'] = opn_coliv_nlp['data'].apply(nltk.word_tokenize)
opn_coliv_nlp['data_tokens'][18] #not outputting tokenized version as it should

opn_coliv_nlp['data_tokens'].shape
...
opn_coliv_nlp['data_tokens'] = opn_coliv_nlp['data_tokens'].apply(remove_short_tokens) #modify based on new columns approach

opn_coliv_nlp['data_tokens'][18]
...
# apply stemming
opn_coliv_nlp['data_stemmed'] = opn_coliv_nlp['data_tokens'].apply(stemming)

# tokens post-stemming
opn_coliv_nlp['data_stemmed'][18]
...
opn_coliv_nlp['data_stopwords'] = opn_coliv_nlp['data_stemmed'].apply(clean_stopwords)

opn_coliv_nlp['data_stopwords'][18] 
...
opn_coliv_nlp['data_eda'] = opn_coliv_nlp['data_stopwords'] 

opn_coliv_nlp['data_eda'][18] #renamed to eda to mark the next phase of analysis
...
opn_coliv_nlp['data_str'] = opn_coliv_nlp['data_eda'].apply(join_tokens)

coliv_words = opn_coliv_nlp['data_str']
...
coliv_freq = []
for item in opn_coliv_nlp['data_eda'] :
...
# NOT WORKING BECAUSE I AM USING AN INCOMPATIBLE VERSION OF PYTHON AND UPGRADING pip3 install Pillow==9.5.0 isn't working   
...
coliv_respns = opn_coliv_nlp['data'] #what's here and how is this utlising the pre-processing and data cleaning?
...
'''Co-occurence Network to visualise the word popularities'''